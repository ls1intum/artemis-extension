...
- **👍 / 👎** = the existing message-helpfulness control (Lucide thumbs, **hover-revealed** in a floating action bar at the card's bottom edge; persisted via the existing `/helpful`). Helpfulness telemetry only — **not** a reject and **not** an annoyance signal. *(2026-06-28, final design: the bar is **absolutely positioned**, so it reserves **no** space in the resting state — fixing the large empty gap a plain `opacity:0` hover-reveal had left — and overhangs the card's bottom edge on hover. It stays a child of the bubble, so hovering the bar keeps the card's `:hover` true (no disappear-on-approach). Thumbs sit on the left.)*
- **Dismiss** = the reject control, in the **same hover bar**, to the **right** of the thumbs. It **collapses/mutes** the bubble + records a dismiss outcome (persisted server-side, §7) + a dismiss strike (§5.2). It does **not** delete the message (§6.3). The per-exercise on/off is NOT on the bubble — it lives in the AskIris control (§12.2). *(2026-06-28, final design: moved from a separate always-visible button into the hover bar at Liam's request — the resting card is clean, both 👍/👎 and Dismiss appear on hover. This deliberately relaxes the earlier "Dismiss must be always-visible" (Horvitz P6/P7) stance in favour of a quieter resting state; the reject stays one hover + one click away, and the always-available off-switch remains in the AskIris control (§12.2).)*
...
**6.5 Repeated proactive hints collapse in the chat (display only, 2026-06-28).** The engine intentionally re-alerts while a struggle persists (§5.1: detection stays neutral, only delivery backs off), so the shared thread can accumulate several near-identical proactive hints about the same situation. To keep the chat readable, a **run of consecutive proactive messages** (no student turn between them) renders as **one card**: the latest suggestion in full, the earlier repeats folded behind a **"Show N earlier suggestions"** toggle (expands to "Hide earlier suggestions"). The toggle is a quiet **chevron + link-style line** (no border), spaced clear of the preceding timestamp — not the boxed/dashed pill of the first cut. A student turn or a normal assistant reply breaks the run, so distinct help episodes the student engaged with stay separate. This is **purely client-side display grouping** (`groupProactiveMessages`); nothing is deleted (§6.3 still holds — every message stays persisted and reachable), and detection/backoff are untouched. Run-collapse is deliberately conservative (consecutiveness, not anchor- or text-matching): the chat-message stream carries no anchor, and consecutive proactive bubbles the student never answered are exactly the redundant nag this targets. `[ENG]`
...
**§12.1 — Bubble affordances:** reply (exists) **+ 👍/👎 (helpfulness) + Dismiss (reject)**. 👍/👎 = the existing helpfulness control (**hover-revealed** in a floating bottom bar with Dismiss to their right, persisted via `/helpful`), telemetry only, **not** a reject and **not** an annoyance increment. **Dismiss** = a reject button in the same hover bar, to the right of the thumbs (2026-06-28 final design, §6.2), that collapses/mutes the bubble (does not delete, §6.3) + records a dismiss outcome (§7.5) + a dismiss strike (§5.2). The per-exercise on/off is NOT on the bubble (see §12.2). (This resolves the earlier §5/§6/§12 contradiction: thumbs ≠ reject; Dismiss = reject ≠ delete.)
...
- an **On/Off switch** carrying a **visible "Proactive help" label** plus a tooltip that states what on/off actually does (so it is legible at a glance, not a bare "On" pill with no context). 2-position slider-style toggle; **not** a multi-level proactivity slider (that is §5.3 future work). Sets the **durable per-exercise** proactivity preference (default On `[ENG]`, gated by the existing AI opt-in; persisted per exercise id).
- a **3-state status badge** that tells the truth: **On** / **paused** / **Off** (explicit). The paused state is the transient §5.2 auto-pause and is surfaced as **"Paused after dismissing recent hints"** (the reason is named explicitly, not a bare "paused"), with a "Resume" action and an explanatory tooltip; it resolves on engagement / new exercise. The switch only ever reflects the *explicit* On/Off; the auto-pause shows as the separate paused note so the switch never moves on its own.
...
- **Card states — one term per situation (consistent with §13/§14):**
  - **Available** (Iris on · course-proactive on · consent ok): the **On / paused / Off** switch+badge above (paused = the dismiss-driven auto-pause, surfaced as "Paused after dismissing recent hints"). "Off" here = the *student's explicit* off; switch flippable.

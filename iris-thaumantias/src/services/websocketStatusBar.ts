 ...
 * Only visible when debug mode is enabled via `artemis.debugMode` setting.
 * 
 * Features:
...
export class WebSocketStatusBarService implements vscode.Disposable {
    ...
    constructor(websocketService: ArtemisWebsocketService) {
        ...
        this._disposables.push(
            vscode.workspace.onDidChangeConfiguration(event => {
                if (event.affectsConfiguration(`${VSCODE_CONFIG.ARTEMIS_SECTION}.${VSCODE_CONFIG.DEBUG_MODE_KEY}`)) {
                    this._log('Debug mode configuration changed');
                    this._updateDebugModeState();
                }
            })
        );
    ...
    /**
     * Check and update the debug mode state from configuration
     */
    private _updateDebugModeState(): void {
        const config = vscode.workspace.getConfiguration(VSCODE_CONFIG.ARTEMIS_SECTION);
        this._isDebugMode = config.get<boolean>(VSCODE_CONFIG.DEBUG_MODE_KEY, false);
        
        this._log(`Debug mode: ${this._isDebugMode}`);

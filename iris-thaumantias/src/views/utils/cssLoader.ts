    ...
    for (const baseCssPath of possiblePaths) {
        if (fs.existsSync(baseCssPath)) {
            try {
                ...
                logger.viewError(`Failed to read base.css from ${baseCssPath}`, error);
            }
        }
    }

    logger.viewWarn('base.css not found in any expected location');
    return '';
}
...
function readCssContent(relativePath: string): string {
    ...
    try {
        ...
        logger.viewError(`Failed to load CSS file: ${cssPath}`, {
            error,
            __dirname,
            viewsBaseDir: viewsDir,
            relativePath
        });
        return '';
    }

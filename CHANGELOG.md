...
- **Chat provider:** Split the Iris chat's webview provider, which had grown to 1528 lines and 49 methods in one class. It now owns only the VS Code surface (the view, the command dispatch, the wiring) and hands the actual behaviour to three collaborators, each owning one question end to end: where the conversation points, what happens to a message that is sent, and whether Iris is usable where the student is. Two duplicated rules disappeared with the move: taking both availability banners down was written out three times, and the guard that stops a slow settings check from labelling the conversation the student has since left was written out twice. No behaviour change.
- **God-file guard:** Lint now fails a file in `src/` over 1000 lines. One file is still above it and is pinned to its exact current length, so it can only shrink (#439).

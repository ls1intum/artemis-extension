...
- **Error messages are readable again.** Their text was the theme's error red on a red-tinted block, which is 4.04:1 at 13px where the accessibility standard asks for 4.5:1. The text is now the theme's normal foreground; the red border and fill still mark it as an error.

- **Course and exercise rows announce themselves correctly.** A screen reader was told each row was one option of a selection, in a list that was never declared as one. They are buttons, which is what they behave like, and the row matching your open workspace is marked as the current one.
...
- **The supported VS Code floor is now guarded by a test.** `@types/vscode` sits at `engines.vscode` so the compiler rejects an API an older VS Code does not have. A test pins that the two agree, and Renovate no longer proposes raising the typings on their own.

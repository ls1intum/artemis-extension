...
function tick(t: number, s: number, v: number, f: FeatureVector): TickRecord {
    return { t, ts: t * 1000, features: f, sBase: s, s, v, fastDecay: false, boundariesPreGate: [], alert: null, decisionTrace: emptyDecisionTrace };
}
...
describe('buildStruggleSignal', () => {
    it('maps alert + ticks to the wire signal and ranks dominant components', () => {
        ...
        const ticks: TickRecord[] = [tick(520, 0.5, 0.6, features({})), tick(530, 0.7, 0.72, f)];
        const alert: EditAlertRecord = { kind: 'edit', t: 530, ts: 530000, urgency: 0.72, v: 0.72, typesPreGate: ['FM'], types: ['FM', 'STATE'], primary: 'FM', path: 'armed', inWarmup: false, inGrace: false };
    ...
    it('tolerates an empty tick buffer (no dominant components, sessionSeconds from alert)', () => {
        const alert: EditAlertRecord = { kind: 'edit', t: 100, ts: 100000, urgency: 0.5, v: 0.5, typesPreGate: ['N1'], types: ['N1'], primary: 'N1', path: 'armed', inWarmup: true, inGrace: false };

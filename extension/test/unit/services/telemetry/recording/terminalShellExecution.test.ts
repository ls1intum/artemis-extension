/**
 * Whitebox tests for the terminal-shell-execution capture path inside
 * ObservationRegistry / SessionRecorder: they seed internal state directly and
 * call private helpers via unsafe casts. The two helpers below wrap the
 * reach-through into `_observation._terminalCollector`.
 */
...
suite('TerminalCollector characterization tests', () => {
    ...
    test('full lifecycle: shellExecStart, async read, shellExecEnd emits terminalCommand with captured output, end fields, and start-time generation', async () => {
        // Monkey-patch vscode.window to capture the listener bodies that
        ...
        // end-listener emission gate, and the generation flow end-to-end.

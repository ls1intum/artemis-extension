...
// Each per-variant parser takes a pre-validated `Record<string, unknown>` (the
// dispatcher has already confirmed `type` is a string and `timestamp` is finite)
// and either returns the typed event or `null` if any required field is malformed.

 ...
 */
export function EpisodeTimeline({ messages, episodeId, dismissable, renderRowBody }: EpisodeTimelineProps) {
            ...
            {messages.map((m) => {
                ...
                return (
                    <div key={m.localId} className={clsx(styles.row, isLatest && styles.rowLast)}>
                        ...
                        <div className={styles.body}>{renderRowBody(m, { kind, state }, dismissable && isLatest)}</div>
                    </div>
                );
            })}
        </div>

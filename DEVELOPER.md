...
- **Clean build** (Open VSX, bundled into EduIDE) - built by `scripts/package-openvsx.js` from a staging directory using `scripts/generate-clean-manifest.js`. The clean variant **excludes** the struggle-detection engine, the recording pipeline, and the data-collection consent flow. In its manifest the `artemis.dataCollectionConsent` and `artemis.struggleDetection.*` settings and the `artemis.openRecordingsFolder` / `artemis.showStruggleScore` commands are **removed entirely** (no absent feature is advertised); `scripts/verify-clean-bundle.js --profile=openvsx` fails the build if any excluded code reappears in the bundle.

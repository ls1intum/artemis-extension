...
- **Clean build** (Open VSX, bundled into EduIDE) - built by `scripts/package-openvsx.js` from a staging directory using `scripts/generate-clean-manifest.js`. The clean variant **excludes** the struggle-detection engine, the recording pipeline, and the data-collection consent flow. The legacy `artemis.struggleDetection.enabled` / `artemis.struggleDetection.showInterventions` settings were removed from the extension entirely (#352), not just from this manifest; only the `artemis.dataCollectionConsent` setting and the `artemis.openRecordingsFolder` / `artemis.showStruggleScore` commands remain profile-specific and are **removed entirely** from the clean manifest (no absent feature is advertised); `scripts/verify-clean-bundle.js --profile=openvsx` fails the build if any excluded code reappears in the bundle.
